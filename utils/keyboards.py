...
def orders_keyboard(orders: list[dict]):
    ...
    for order in orders:
        ...
        label = f"🚚 {preview[:30]}{'...' if len(preview)>30 else ''} → Rp{order['reward_coins']:,}"
...
def land_keyboard(obstacles: list[dict], plots: list[dict]):
    ...
    for obs_slot, obs_data in obs_slots.items():
        ...
        buttons.append([InlineKeyboardButton(
            f"{obs['emoji']} Slot {obs_slot+1}: {obs['name']} (butuh {get_item_emoji(obs['tool'])} {get_item_name(obs['tool'])})",
            callback_data=f"clear_{obs_slot}"
        )])

    if not obs_slots:
        buttons.append([InlineKeyboardButton("✅ Tidak ada rintangan!", callback_data="noop")])

    buttons.append([
        InlineKeyboardButton("🔧 Perluas Farm", callback_data="expand_farm"),
        InlineKeyboardButton("🐾 Perluas Kandang", callback_data="expand_pens"),
    ])
    buttons.append([InlineKeyboardButton("🏠 Menu Utama", callback_data="menu")])
    return InlineKeyboardMarkup(buttons)
